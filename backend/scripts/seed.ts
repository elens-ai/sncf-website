...
async function insert(slug:string,key:string,raw:Record<string,any>,order=0){
  const found=await payload.find({collection:slug as CollectionSlug,where:{key:{equals:key}},depth:0,limit:1,overrideAccess:true})
  if(found.totalDocs){skipped++;return}
  const {id,...data}=raw
