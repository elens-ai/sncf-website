...
export function EditorialTimeline({ items }: { items: { year: string; text: string; label: string; href: string }[] }) {
  return <ol className="ed-timeline" aria-label={getCMSCopy("copy.EditorialContent.7ea80acf5f85", "Foundation milestones")} data-reveal>{items.map((item,i) => <li key={item.year}><span className="ed-timeline-dot" aria-hidden="true"/><p className="ed-eyebrow">{getCMSCopy("copy.EditorialContent.5feceb66ffc8", "0")}{i + 1} / {item.label}</p><h3>{item.year}</h3><p>{item.text}</p><a className="ed-link" href={item.href}>{getCMSCopy("copy.EditorialContent.fbcc95dc9ecc", "Explore this chapter ")}<ArrowUpRight size={16}/></a></li>)}</ol>;
}
