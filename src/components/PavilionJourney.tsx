...
export const PavilionJourney: React.FC = () => {
  ...
  useEffect(() => {
    ...
    import.meta.hot?.accept('./PavilionScene', module => {
      ...
      scene.current = module.createPavilion(host.current); update();
    });
    ...
    const read = () => {
      ...
      progress.current = Math.min(5, pavilionProgress(-rect.top / Math.max(1, rect.height - innerHeight)));
      ...
      setApproachingFinale(progress.current >= 4.2);
      const exit = Math.max(0, Math.min(1, (progress.current - 4) / .2));
      el.style.setProperty('--projects-copy-opacity', String(1 - exit * exit * (3 - 2 * exit)));
  ...
  const go = (index: number) => {
    ...
    window.scrollTo({ top: scrollY + el.getBoundingClientRect().top + (el.offsetHeight - innerHeight) * pavilionScrollFraction(destination), behavior: calm.current ? 'instant' : 'smooth' });
  };
  return <section id="pillars-section" className="pavilion-track" data-pavilion="true" ref={track} aria-label="Our work — an immersive journey">
    <div className="pavilion-stage" data-exhibit-side={step === 2 || step === 4 ? 'left' : 'right'} data-gallery={galleryIndex >= 0} data-farewell={step === 5 || approachingFinale}>
      ...
      {step === 0 ? <div className="pavilion-welcome" key="welcome">
        ...
        <button className="pavilion-enter" onClick={() => {
          const el = track.current!;
          window.scrollTo({ top: scrollY + el.getBoundingClientRect().top + (el.offsetHeight - innerHeight) * pavilionScrollFraction(.125), behavior: calm.current ? 'instant' : 'smooth' });
        }}><span><ArrowDown size={21} /></span>Step inside</button>
      </div> : step === 5 ? <div className="pavilion-welcome pavilion-farewell" key="farewell">
        <p className="pavilion-eyebrow">Sant Nirankari Charitable Foundation</p><h2>Thank You for Visiting</h2><p>The journey continues in every act of kindness.</p><button className="pavilion-text-link" onClick={() => document.getElementById('events-section')?.scrollIntoView({ behavior: calm.current ? 'instant' : 'smooth' })}>Discover what’s next <ArrowUpRight size={17} /></button>
      </div> : approachingFinale ? null : galleryIndex >= 0 ? <div className="pavilion-gallery-caption" key={`${step}-${galleryIndex}`}>
