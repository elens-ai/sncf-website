...
for (const [width, height] of [[360,800],[390,844],[768,1024],[844,390]]) test(`pavilion photo and finale fit ${width}×${height}`, () => {
  ...
  if (height < width) assert.ok(frame.photoFOV >= 68, 'landscape photo has vertical breathing room');
  assert.ok(frame.finaleTextWidth / 2048 * 14.4 < finaleWidth, 'final text fits horizontal view');
});
test('desktop camera and emblem retain their original composition', () => {
  ...
  assert.equal(frame.emblemScale, 1);
});
