...
export function pavilionFraming(width: number, height: number, fieldOfView: number, emblemSize = 2.4, emblemHeight = 4.35) {
  ...
  const emblemFraction = Math.min(.30, Math.max(.18, (height - 180) / height * .48));
  const emblemScale = Math.min(1, finaleViewWidth * .76 / emblemSize, emblemViewHeight * emblemFraction / emblemSize);
  ...
  return {
    ...
    emblemScale,
    emblemOffsetY: 3.5 + (.5 - emblemScreenCentre) * emblemViewHeight - emblemHeight,
